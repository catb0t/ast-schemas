...
from ompy.test.util import read_segmented_om, all_eq, suiteFactory, caseFactory
...
class Test_MyTests(unittest.TestCase):
    def test_static(self):
        ...
        for file in sorted(STATIC_DIR.glob("*.json")):
            ...
            self.assertTrue(
                OmValidator.is_valid(test_json),
                "the test AST must follow the schema",
            )
            self.assertTrue(
                OmValidator.is_valid(computed_json),
            ...
            self.assertEqual(
                computed_om, original_om, "the deparsed Om should match the test Om"
            )
            ...
            print(
                "\t- Om correctly parses to JSON:", ("ok" if json_same else "nope")
            )
            print(
                "\t- parsed JSON correctly deparses to Om:",
                ("ok" if om_same else "nope"),
            )
    ...
    def test_normalized(self):
        ...
        for file in sorted(NORM_DIR.glob("*.json")):
            ...
            reparse_strip_fact_json = parser.parse(
                parser.deparse(fact_json), strip=True
            )
            ...
            parsed_fact_om_big, parsed_fact_om_optimal = map(
                parser.parse, fact_om
            )  # noqa

            stripped_fact_om, stripped_fact_om_optimal = map(
                lambda x: parser.parse(x, strip=True), fact_om
            )

            filtered_stripped_fact_om = parser.filter_separators(
                stripped_fact_om
            )  # noqa
            ...
            all_eq(
                ...
                [
                    ...
                    stripped_fact_om,
                    stripped_fact_om_optimal,
                    filtered_stripped_fact_om,
                    filtered_fact_om_big,
                ],  # noqa
            )
    ...
    def test_illformed(self):
        ...
        for file in sorted(ILLFORMED_DIR.glob("*.om")):
            ...
            self.assertEqual([exc.reason.value, exc.line, exc.col], expect_vals)
            print("-", file.stem)
