...
def _unbalanced_braces(source):
    ...
    result is boolean
    """
    return _count_found(unesc_opens, source) != _count_found(unesc_closes, source)
...
def _create_context(source, idx):
    ...
    return (
        opens_before[-2 if len(opens_before) > 1 else 0].span()[0],
        idx + closes_after[1 if len(closes_after) > 1 else 0].span()[1],
    )
...
def _parse_to_tree(
    source, fname, strip=False, skip_to_idx=DoNotSkip, depth=0, line=1, col=1
):
    ...
    for idx, ch in enumerate(source):
        ...
        if not escape_next and ch in SEP_STR:
            ...
            if strip:
                ...
                # {{A} B} { {A} {B} } etc
                # are in {A  A}
                if not (_node_is(last_id, Node.OPERATOR) and next_ch not in NOT_OP):
                    continue

            last_id = Node.SEPARATOR
            building.append(_create_node(Node.SEPARATOR, SEP_STR.index(ch)))
