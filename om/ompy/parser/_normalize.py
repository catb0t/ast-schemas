...
def getfunc(text_mode):
    def _src_test(ch):
        return ch in SEP_STR

    def _obj_test(elt):
        return _node_is(elt, Node.SEPARATOR)
    ...
    def _src_next(ch):
        return Node.OPERAND if ch in NOT_OP else Node.OPERATOR
...
def _shrink_separator_span(text_mode, form, span_begin_idx, previous_node=NoLastID):
    """
...
def filter_separators(form, recurse=True):
    ...
    for i, e in enumerate(form):
        ...
        if _node_is(e, Node.SEPARATOR):
            ...
            if recurse:
                val = filter_separators(e["value"])
            else:
                val = e['value']
            new_form.append(
                _create_node(Node.OPERAND, val)
            )
...
def normalize_separators(form):
    ...
    for node in form:
        if _node_is(node, Node.SEPARATOR):
            new_form.append(_create_node(Node.SEPARATOR, 1))
        elif _node_is(node, Node.OPERAND):
            new_form.append(
                _create_node(Node.OPERAND, normalize_separators(node["value"]))
            )
