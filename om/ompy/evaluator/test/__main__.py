    ...
    def test_operators(self):
        ...
        for d in sorted(OPERATORS_DIR.iterdir()):
            if d.name not in ("quote", "dequote"):
                print("NOTE: only testing `quote` and `dequote` for now")
                continue
            print("-", d.stem)
            for file in sorted(d.glob("*.om")):
                ...
                self.assertEqual(
                    expect, result, "result of evaluation should match the test"
                )

                print("\t- Om evaluates correctly")
                deparse_result, deparse_expect = (deparse(result), deparse(expect))
