...
def transform_operand(elt, bindings):
    return see_reversed(transform1(see_reversed(elt["value"]), up_bindings=bindings))
...
def transform1(prog, up_bindings=None):
    ...
    for elt in prog:
        ...
        if elt["id"] == "operator":
            ...
            new_prog.append(
                {"id": "operand", "value": transform_operand(elt, local_bindings)}
            )
...
def _evaluate(prog, up_bindings=None):
    ...
    return see_reversed(
        transform1(see_reversed(filter_separators(prog)), operations)
    )
